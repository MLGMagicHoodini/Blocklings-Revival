...
{
    /**
     * The index of the slot containing the blockling's main InteractionHand tool.
     */
    public static final int TOOL_MAIN_HAND = 0;

    /**
     * The index of the slot containing the blockling's off InteractionHand tool.
     */
    ...
    /**
     * @return which InteractionHand(s) the blockling currently has a tool in.
     */
    @Nonnull
    ...
    /**
     * @return true if the blockling has a tool equipped in the given InteractionHand.
     */
    public boolean hasToolEquipped(@Nonnull InteractionHand InteractionHand)
    {
        return ToolUtil.isTool(getHandStack(InteractionHand));
    }
    ...
    /**
     * @return true if the blockling has a tool equipped of the given type in the given InteractionHand.
     */
    public boolean hasToolEquipped(@Nonnull InteractionHand InteractionHand, @Nonnull ToolType toolType)
    {
        return toolType.is(getHandStack(InteractionHand));
    }

    /**
     * @return the stack in the given InteractionHand.
     */
    @Nonnull
    public ItemStack getHandStack(@Nonnull InteractionHand InteractionHand)
    {
        return InteractionHand == InteractionHand.MAIN_HAND ? getItem(TOOL_MAIN_HAND) : getItem(TOOL_OFF_HAND);
    }

    /**
     * Sets the stack in the given InteractionHand.
     */
    public void setHandStack(@Nonnull InteractionHand InteractionHand, @Nonnull ItemStack stack)
    {
    ...
    /**
     * @return true if the blockling is attacking with the given InteractionHand.
     */
    public boolean isAttackingWith(@Nonnull BlocklingHand InteractionHand)
    {
        BlocklingHand attackingHand = findAttackingHand();

        if (InteractionHand == BlocklingHand.BOTH && attackingHand == BlocklingHand.BOTH)
        {
            return true;
        }
        else if (InteractionHand == BlocklingHand.MAIN && (attackingHand == BlocklingHand.BOTH || attackingHand == BlocklingHand.MAIN))
        {
            return true;
        }
        else if (InteractionHand == BlocklingHand.OFF && (attackingHand == BlocklingHand.BOTH || attackingHand == BlocklingHand.OFF))
        {
            return true;
        }
        else if (InteractionHand == BlocklingHand.NONE && attackingHand == BlocklingHand.NONE)
        {
            return true;
        }
    ...
    /**
     * @return the InteractionHand the blockling is attacking with.
     */
    @Nonnull
    ...
    /**
     * @return true if the given InteractionHand has a weapon equipped that can currently be used.
     */
    public boolean hasUseableWeapon(@Nonnull InteractionHand InteractionHand)
    {
        return hasToolEquipped(InteractionHand, ToolType.WEAPON) && ToolUtil.isUseableTool(getHandStack(InteractionHand));
    }

    /**
     * @return true if the given InteractionHand has a tool equipped that can currently be used.
     */
    public boolean hasUseableTool(@Nonnull InteractionHand InteractionHand)
    {
        return ToolUtil.isUseableTool(getHandStack(InteractionHand));
    }
    ...
    {
        return ToolUtil.canToolHarvest(getHandStack(InteractionHand), blockState);
    }

    /**
     * @return true if any slots were switched.
     */
    public boolean trySwitchToBestTool(@Nonnull BlocklingHand InteractionHand, @Nonnull ToolContext context)
    {
        Pair<SwitchedTools, SwitchedTools> bestToolSlots = findBestToolSlotsToSwitchTo(InteractionHand, context);
    ...
    /**
     * @return the best tools for the given InteractionHand slots.
     */
    @Nonnull
    public Pair<ItemStack, ItemStack> findBestToolsToSwitchTo(@Nonnull BlocklingHand InteractionHand, @Nonnull ToolContext context)
    {
        Pair<SwitchedTools, SwitchedTools> bestTools = findBestToolSlotsToSwitchTo(InteractionHand, context);

        return new MutablePair<>(getItem(bestTools.getKey().bestSlot), getItem(bestTools.getValue().bestSlot));
    }

    /**
     * @return the current InteractionHand slots and the best InteractionHand slots to switch to.
     */
    @Nonnull
    public Pair<SwitchedTools, SwitchedTools> findBestToolSlotsToSwitchTo(@Nonnull BlocklingHand InteractionHand, @Nonnull ToolContext context)
    {
        if (InteractionHand == BlocklingHand.MAIN)
        {
        ...
        else if (InteractionHand == BlocklingHand.OFF)
        {
        ...
        else if (InteractionHand == BlocklingHand.BOTH)
        {
    ...
    /**
     * @param InteractionHand the InteractionHand to find the tool for, should be either MAIN or OFF.
     * @param context the context to use when finding the best tool.
     * @return a pair containing the slots to swap with the best items, InteractionHand slot first then the other slot to swap with.
     */
    @Nonnull
    public SwitchedTools findBestToolSlotToSwitchTo(@Nonnull BlocklingHand InteractionHand, @Nonnull ToolContext context)
    {
        if (!(InteractionHand == BlocklingHand.MAIN || InteractionHand == BlocklingHand.OFF))
        {
            BlocklingsConstants.LOG.warn("Tried to find the best tool to switch to with a InteractionHand that wasn't MAIN or OFF!");

            return new SwitchedTools(TOOL_OFF_HAND, TOOL_OFF_HAND);
        }

        int bestSlot = InteractionHand == BlocklingHand.MAIN ? TOOL_MAIN_HAND : TOOL_OFF_HAND;
