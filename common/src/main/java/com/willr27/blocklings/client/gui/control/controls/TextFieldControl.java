...
public class TextFieldControl extends Control
{
    ...
    public void onRender(@Nonnull GuiGraphics matrixStack, @Nonnull ScissorStack scissorStack, double mouseX, double mouseY, float partialTicks)
    {
        ...
        try
        {
            ...
            z = ReflectionHelper.getPrivateValue(Matrix4f.class, matrixStack.pose().last().pose(), "m23");
        }
        catch (Exception ex)
        {
//            Blocklings.LOGGER.warn(ex.toString());
        ...
        GuiGraphics textFieldGuiGraphics = new GuiGraphics(Minecraft.getInstance(), Minecraft.getInstance().renderBuffers().bufferSource());
        textFieldGuiGraphics.pose().translate(textFieldXRemainder, textFieldYRemainder, z);
        textFieldWidget.render(textFieldGuiGraphics, (int) Math.round(mouseX), (int) Math.round(mouseY), partialTicks);
