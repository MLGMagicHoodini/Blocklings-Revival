...
public class StatsScreen extends TabbedScreen
{
    ...
    public StatsScreen(@Nonnull BlocklingEntity blockling)
    {
        ...
        textFieldControl.eventBus.subscribe((BaseControl control, FocusChangedEvent e) ->
        {
            ...
            {
                Component name = BlocklingsRegistries.blocklingItem().getName(BlocklingsRegistries.blocklingItemStack());
        ...
        EnumeratingControl combatStats = new EnumeratingStatControl(BlocklingsTranslationTextComponent.create("stats.attack.name"));
        ...
        EnumeratingControl defenceStats = new EnumeratingStatControl(BlocklingsTranslationTextComponent.create("stats.defence.name"));
        ...
        EnumeratingControl gatherStats = new EnumeratingStatControl(BlocklingsTranslationTextComponent.create("stats.gathering.name"));
        ...
        EnumeratingControl movementStats = new EnumeratingStatControl(BlocklingsTranslationTextComponent.create("stats.movement.name"));
        ...
        EntityControl entityControl = new EntityControl()
        {
            ...
            public void onRenderTooltip(@Nonnull GuiGraphics matrixStack, double mouseX, double mouseY, float partialTicks)
            {
                ...
                tooltip.add(Component.literal(ChatFormatting.GRAY + BlocklingsTranslationTextComponent.create("type.natural.name").getString() + ChatFormatting.WHITE + blockling.getNaturalBlocklingType().name.getString()).getVisualOrderText());

                List<String> splitText;

                if (GuiUtil.get().isCrouchKeyDown())
                {
                    splitText = GuiUtil.get().split(BlocklingsTranslationTextComponent.create("type.natural.desc").getString(), 200);
                ...
                if (blockling.isTame())
                {
                    tooltip.add(Component.literal(ChatFormatting.GREEN + BlocklingsTranslationTextComponent.create("taming.already_tamed").getString()).getVisualOrderText());
                }
                else
                {
                    tooltip.add(Component.literal(ChatFormatting.YELLOW + BlocklingsTranslationTextComponent.create("taming.hint").getString()).getVisualOrderText());
                }

                if (GuiUtil.get().isCrouchKeyDown())
                {
                    BlocklingType naturalType = blockling.getNaturalBlocklingType();
                    String evolveFoods = BlocklingType.TYPES.stream()
                            .filter(type -> type != naturalType)
                            .map(type -> type.name.getString() + ": " + type.getRepresentativeFoodItem().getDescription().getString())
                            .collect(joining(", "));

                    if (!evolveFoods.isEmpty())
                    {
                        String evolveFoodsString = ChatFormatting.GRAY + BlocklingsTranslationTextComponent.create("type.foods.evolve_type").getString() + ChatFormatting.WHITE + evolveFoods;
                        splitText = GuiUtil.get().split(evolveFoodsString, 200);
                        splitText.stream().map(s -> Component.literal(s).getVisualOrderText()).forEach(tooltip::add);
                    }

                    BlocklingType primaryType = blockling.getBlocklingType();
                    String typeChangeFoods = BlocklingType.TYPES.stream()
                            .filter(type -> type != primaryType)
                            .map(type -> type.name.getString() + ": " + type.getRepresentativeFoodItem().getDescription().getString())
                            .collect(joining(", "));

                    if (!typeChangeFoods.isEmpty())
                    {
                        String changeFoodsString = ChatFormatting.GRAY + BlocklingsTranslationTextComponent.create("type.foods.change_type").getString() + ChatFormatting.WHITE + typeChangeFoods;
                        splitText = GuiUtil.get().split(changeFoodsString, 200);
                        splitText.stream().map(s -> Component.literal(s).getVisualOrderText()).forEach(tooltip::add);
                    }
                }
                ...
                if (!GuiUtil.get().isCrouchKeyDown())
                {
                    tooltip.add(Component.literal(ChatFormatting.DARK_GRAY + "" + ChatFormatting.ITALIC + BlocklingsTranslationTextComponent.create("gui.more_info", Minecraft.getInstance().options.keyShift.getTranslatedKeyMessage().getString()).getString()).getVisualOrderText());
                }
        ...
        entityControl.setFaceCamera(true);
