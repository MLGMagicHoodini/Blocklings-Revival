...
public class SkillsPanel extends CanvasPanel
{
    ...
    private class SkillControl extends Control
    {
        ...
        public SkillControl(@Nonnull Skill skill, @Nonnull ConfirmationControl confirmationControl)
        {
            ...
            TexturedControl type = new TexturedControl(skill.info.general.type.texture)
            {
                ...
                protected void onRender(@Nonnull GuiGraphics matrixStack, @Nonnull ScissorStack scissorStack, double mouseX, double mouseY, float partialTicks)
                {
                    matrixStack.pose().translate(0.0, 0.0, 0.2);
        ...
        private void recreateInfo()
        {
            ...
            TextBlockControl nameText = new TextBlockControl()
            {
                ...
                protected void onRenderUpdate(@Nonnull GuiGraphics matrixStack, @Nonnull ScissorStack scissorStack, double mouseX, double mouseY, float partialTicks)
                {
                    ...
                    {
                        setText(BlocklingsTranslationTextComponent.create("skill.unknown"));
                    }
                }
            };
        ...
        private List<String> getSkillDescription()
        {
            ...
            if (state == Skill.State.LOCKED)
            {
                name = BlocklingsTranslationTextComponent.create("skill.unknown").getString();
                description.clear();
                description.add("...");
            }
            else
            {
                ...
                {
                    description.add("");
                    description.add(BlocklingsTranslationTextComponent.create("requirements").getString());
                ...
                {
                    description.add("");
                    description.add(BlocklingsTranslationTextComponent.create("conflicts").getString());
    ...
    private class PathControl extends Control
    {
        ...
        protected void onRender(@Nonnull GuiGraphics matrixStack, @Nonnull ScissorStack scissorStack, double mouseX, double mouseY, float partialTicks)
        {
            ...
            if (path.size() > 1)
            {
                for (int i = 0; i < path.size() - 1; i++)
                {
                    ...
                    matrixStack.pose().pushPose();
                    matrixStack.pose().translate(0.0, 0.0, 0.1);
         ...
         * @param partialTicks
         */
        private void renderHighlight(@Nonnull GuiGraphics matrixStack, @Nonnull ScissorStack scissorStack, double mouseX, double mouseY, float partialTicks)
        {
            double outerLineWidth = outerLineSize * getPixelScaleX();
            double outerLineHeight = outerLineSize * getPixelScaleY();
            double innerLineWidth = innerLineSize * getPixelScaleX();
    ...
    private class ConfirmationControl extends Control
    {
        ...
        public ConfirmationControl()
        {
            ...
            yesButton.textBlock.setText(BlocklingsTranslationTextComponent.create("skill.buy.yes"));
            ...
            noButton.textBlock.setText(BlocklingsTranslationTextComponent.create("skill.buy.no"));
        ...
        public void setWidth(double width)
        {
            ...
            if (skill != null)
            {
                ...
                for (String line : GuiUtil.get().split(BlocklingsTranslationTextComponent.create("skill.buy_confirmation", ChatFormatting.LIGHT_PURPLE + skill.info.general.name.getString() + ChatFormatting.WHITE).getString(), (int) getWidth() - 20))
                {
