...
public class BlockSideSelectionControl extends BlockControl
{
    ...
    protected void onRender(@Nonnull GuiGraphics matrixStack, @Nonnull ScissorStack scissorStack, double mouseX, double mouseY, float partialTicks)
    {
        ...
        for (int i = 0; i < selectedDirections.size(); i++)
        {
            ...
            {
                case NORTH:
                    renderTextOnSide(s, x, y, z, scale, new Quaternionf(), 0xffffffff);
                    break;
                case EAST:
                    renderTextOnSide(s, x, y, z, scale, new org.joml.Quaternionf().rotateY((float)Math.toRadians(90.0f)), 0xffffffff);
                    break;
                case SOUTH:
                    renderTextOnSide(s, x, y, z, scale, new org.joml.Quaternionf().rotateY((float)Math.toRadians(180.0f)), 0xffffffff);
                    break;
                case WEST:
                    renderTextOnSide(s, x, y, z, scale, new org.joml.Quaternionf().rotateY((float)Math.toRadians(-90.0f)), 0xffffffff);
                    break;
                case UP:
                    renderTextOnSide(s ,x, y, z, scale, new org.joml.Quaternionf().rotateX((float)Math.toRadians(90.0f)), 0xffffffff);
                    break;
                case DOWN:
                    renderTextOnSide(s, x, y, z, scale, new org.joml.Quaternionf().rotateX((float)Math.toRadians(-90.0f)), 0xffffffff);
                    break;
        ...
        if (mouseOverDirection != null)
        {
            ...
            {
                case NORTH:
                    renderRectangleOnSide(x, y, z, scale, new Quaternionf(), 0x55ffffff);
                    break;
                case SOUTH:
                    renderRectangleOnSide(x, y, z, scale, new org.joml.Quaternionf().rotateY((float)Math.toRadians(180.0f)), 0x55ffffff);
                    break;
                case WEST:
                    renderRectangleOnSide(x, y, z, scale, new org.joml.Quaternionf().rotateY((float)Math.toRadians(-90.0f)), 0x55ffffff);
                    break;
                case EAST:
                    renderRectangleOnSide(x, y, z, scale, new org.joml.Quaternionf().rotateY((float)Math.toRadians(90.0f)), 0x55ffffff);
                    break;
                case UP:
                    renderRectangleOnSide(x, y, z, scale, new org.joml.Quaternionf().rotateX((float)Math.toRadians(90.0f)), 0x55ffffff);
                    break;
                case DOWN:
                    renderRectangleOnSide(x, y, z, scale, new org.joml.Quaternionf().rotateX((float)Math.toRadians(-90.0f)), 0x55ffffff);
                    break;
    ...
    {
        org.joml.Vector3f segmentMinusPlanePoint = new org.joml.Vector3f(segmentPoint.x - planePoint.x, segmentPoint.y - planePoint.y, segmentPoint.z - planePoint.z);
        org.joml.Vector3f planeNormal3f = new org.joml.Vector3f(planeNormal.x, planeNormal.y, planeNormal.z);

        return segmentMinusPlanePoint.dot(planeNormal3f);
    }

    private void renderTextOnSide(String text, float x, float y, float z, float scale, @Nonnull Quaternionf rotation, int colour)
    {
        RenderSystem.disableCull();
        GuiGraphics matrixStack = new GuiGraphics(Minecraft.getInstance(), Minecraft.getInstance().renderBuffers().bufferSource());
        matrixStack.pose().translate(x, y, z);
        matrixStack.pose().mulPose(new Quaternionf(rotationQuat.x, -rotationQuat.y, rotationQuat.z, -rotationQuat.w));
        matrixStack.pose().mulPose(rotation);
        matrixStack.pose().scale(scale / 24.0f, scale / 24.0f, 1.0f);
        matrixStack.pose().translate(-GuiUtil.get().getTextWidth(text) / 2.0, -GuiUtil.get().getLineHeight() / 2.0, scale / 2.0 + 1.0 -0.03);
        RenderSystem.enableDepthTest();
        renderShadowedText(matrixStack, Component.literal(text).getVisualOrderText(), 0, 0, colour);
        RenderSystem.enableCull();
    }

    private void renderRectangleOnSide(float x, float y, float z, float scale, @Nonnull Quaternionf rotation, int colour)
    {
        GuiGraphics matrixStack = new GuiGraphics(Minecraft.getInstance(), Minecraft.getInstance().renderBuffers().bufferSource());
        matrixStack.pose().translate(x, y, z);
        matrixStack.pose().mulPose(new Quaternionf(rotationQuat.x, -rotationQuat.y, rotationQuat.z, -rotationQuat.w));
        matrixStack.pose().mulPose(rotation);
        matrixStack.pose().translate(0.0f, 0.0f, scale / 2 + 0.01);
        RenderSystem.enableDepthTest();
    ...
    public void onRenderTooltip(@Nonnull GuiGraphics matrixStack, double mouseX, double mouseY, float partialTicks)
    {
        ...
        if (mouseOverDirection != null)
        {
            ...
            {
                case NORTH:
                    renderTooltip(matrixStack, mouseX, mouseY, BlocklingsTranslationTextComponent.create("direction.front"));
                    break;
                case SOUTH:
                    renderTooltip(matrixStack, mouseX, mouseY, BlocklingsTranslationTextComponent.create("direction.back"));
                    break;
                case WEST:
                    renderTooltip(matrixStack, mouseX, mouseY, BlocklingsTranslationTextComponent.create("direction.left"));
                    break;
                case EAST:
                    renderTooltip(matrixStack, mouseX, mouseY, BlocklingsTranslationTextComponent.create("direction.right"));
                    break;
                case UP:
                    renderTooltip(matrixStack, mouseX, mouseY, BlocklingsTranslationTextComponent.create("direction.top"));
                    break;
                case DOWN:
                    renderTooltip(matrixStack, mouseX, mouseY, BlocklingsTranslationTextComponent.create("direction.bottom"));
                    break;
            }
        }
    }
    ...
    public Direction getDirectionMouseIsOver()
    {
        ...
        PoseStack pointStack = new PoseStack();
        pointStack.translate(x, y, z);
        pointStack.mulPose(new Quaternionf(rotationQuat));
        PoseStack normalStack = new PoseStack();
        normalStack.mulPose(new Quaternionf(rotationQuat));
        ...
        frontPlanePoint.mul(pointStack.last().pose());
        frontPlaneNormal.mul(normalStack.last().pose());
        backPlanePoint.mul(pointStack.last().pose());
        backPlaneNormal.mul(normalStack.last().pose());
        topPlanePoint.mul(pointStack.last().pose());
        topPlaneNormal.mul(normalStack.last().pose());
        bottomPlanePoint.mul(pointStack.last().pose());
        bottomPlaneNormal.mul(normalStack.last().pose());
        leftPlanePoint.mul(pointStack.last().pose());
        leftPlaneNormal.mul(normalStack.last().pose());
        rightPlanePoint.mul(pointStack.last().pose());
        rightPlaneNormal.mul(normalStack.last().pose());
