...
public class BlocklingTasks implements IReadWriteNBT
{
    ...
    public BlocklingTasks(@Nonnull BlocklingEntity blockling)
    {
        ...
        this.goalSelector = blockling.getGoalSelector();
        this.targetSelector = blockling.getTargetSelector();
    ...
    @Nullable
    public Task getTaskOrLog(@Nullable UUID id, @Nonnull String context)
    {
        if (id == null)
        {
            Blocklings.LOGGER.warn("{} referenced null task id on blockling {}", context, blockling.getUUID());
            return null;
        }
        ...
        if (task == null)
        {
            Blocklings.LOGGER.warn("{} referenced missing task {} on blockling {}", context, id, blockling.getUUID());
        }
    ...
    {
        UUID resolvedId = taskId != null ? taskId : UUID.randomUUID();

        if (getTask(resolvedId) == null)
        {
            Task task = new Task(resolvedId, type, blockling, this);
        ...
        if (sync)
        {
            new TaskCreateMessage(blockling, type.id, resolvedId).sync();
        }
    }
    ...
    /**
     * @return true if a configured task of the given type exists and is not disabled.
     */
    public boolean isTaskTypeEnabled(@Nonnull TaskType type)
    {
        for (Task task : prioritisedTasks)
        {
            if (task.getType() == type && task.isConfigured() && task.getGoal() != null)
            {
                return task.getGoal().getState() != BlocklingGoal.State.DISABLED;
            }
        }
    ...
    /**
     * @return true if any higher-priority task is currently active.
     */
    public boolean hasHigherPriorityActiveTask(@Nonnull Task task)
    {
        int priority = getTaskPriority(task);

        for (Task other : prioritisedTasks)
        {
            if (getTaskPriority(other) >= priority)
            {
                continue;
            }

            if (!other.isConfigured() || other.getGoal() == null)
            {
                continue;
            }

            if (other.getGoal().getState() == BlocklingGoal.State.ACTIVE)
            {
                return true;
            }
        }
