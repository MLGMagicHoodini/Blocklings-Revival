...
/**
 * Configures a float range property (e.g. min leaves-to-logs for woodcutting).
 */
    ...
    public BaseControl createControl()
    {
        FloatRangeControl range = new FloatRangeControl(min, max, value)
        {
            ...
            public void setValue(@Nonnull Float value, boolean updateGrabberPosition, boolean postEvent)
            {
                ...
                // Persist + sync only when the user commits (not every drag tick).
                if (postEvent)
                {
        ...
        range.valueFieldControl.setWidth(26.0);
