...
public abstract class GoalMessage<T extends BlocklingMessage<T>, G> extends BlocklingMessage<T>
{
    ...
    {
        var task = blockling.getTasks().getTask(taskId);
        if (task == null)
        {
            Blocklings.LOGGER.warn("Ignoring goal message for unknown task {} on blockling {}", taskId, blockling.getUUID());
            return;
        }
        ...
        if (goal == null)
        {
            Blocklings.LOGGER.warn("Ignoring goal message for task {} without goal on blockling {}", taskId, blockling.getUUID());
            return;
        }
