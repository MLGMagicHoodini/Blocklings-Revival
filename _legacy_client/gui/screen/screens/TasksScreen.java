    ...
    public TasksScreen(@Nonnull BlocklingEntity blockling)
    {
        ...
        TexturedControl addTaskButton = new TexturedControl(Textures.Common.PLUS_ICON)
        {
            ...
            public void onRenderTooltip(@Nonnull GuiGraphics matrixStack, double mouseX, double mouseY, float partialTicks)
            {
                ...
                tooltip.add(new BlocklingsTranslationTextComponent("task.ui.add").withStyle(blockling.getTasks().isTaskListFull() ? ChatFormatting.GRAY : ChatFormatting.WHITE).getVisualOrderText());
                tooltip.add(new BlocklingsTranslationTextComponent("task.ui.task_amount", blockling.getTasks().getPrioritisedTasks().size(), BlocklingTasks.MAX_TASKS).withStyle(ChatFormatting.GRAY).getVisualOrderText());
