...
public class BlockControl extends Control
{
    ...
    {
        super();
        rotationQuat.mul(org.joml.Vector3f.XP.rotationDegrees(30.0f));
        rotationQuat.mul(org.joml.Vector3f.YP.rotationDegrees(45.0f));
    }
    ...
    protected void onRender(@Nonnull GuiGraphics matrixStack, @Nonnull ScissorStack scissorStack, double mouseX, double mouseY, float partialTicks)
    {
        ...
        {
            Quaternion quat = org.joml.Vector3f.YP.rotationDegrees((float) (mouseDeltaX * getPixelScaleX()) * 0.4f);
            quat.mul(org.joml.Vector3f.XP.rotationDegrees((float) (mouseDeltaY * getPixelScaleY()) * 0.4f));
...
//            Blocklings.LOGGER.warn(ex.toString());
        ...
        GuiGraphics blockGuiGraphics = new GuiGraphics();
        blockGuiGraphics.translate(extraX, -extraY, 0.0);
        blockGuiGraphics.mulPose(rotationQuat);
        blockGuiGraphics.mulPose(org.joml.Vector3f.YP.rotationDegrees(getBlockState().getRenderShape() == BlockRenderType.MODEL ? 180.0f : 0.0f));
        blockGuiGraphics.translate(-0.5f, -0.5f, -0.5f);

        BlockRendererDispatcher blockRenderer = Minecraft.getInstance().getBlockRenderer();
        IRenderTypeBuffer.Impl irendertypebuffer$impl = Minecraft.getInstance().renderBuffers().bufferSource();
        blockRenderer.renderSingleBlock(getBlockState(), blockGuiGraphics, irendertypebuffer$impl, 15728880, OverlayTexture.NO_OVERLAY);
