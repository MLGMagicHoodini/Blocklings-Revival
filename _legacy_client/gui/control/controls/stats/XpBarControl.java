...
public class XpBarControl extends Control
{
    ...
    public void onRenderTooltip(@Nonnull GuiGraphics matrixStack, double mouseX, double mouseY, float partialTicks)
    {
        ...
        tooltip.add(Component.literal(ChatFormatting.GRAY + new BlocklingsTranslationTextComponent("gui.current_level", ChatFormatting.WHITE, level.getValue()).getString()).getVisualOrderText());
        tooltip.add(Component.literal(ChatFormatting.GRAY + new BlocklingsTranslationTextComponent("gui.xp_required", ChatFormatting.WHITE, blockling.getStats().getLevelXpAttribute(this.level).getValue(), BlocklingAttributes.getXpForLevel(level.getValue())).getString()).getVisualOrderText());
