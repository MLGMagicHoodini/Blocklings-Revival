...
import org.joml.org.joml.Vector3f;
...
public class BlockSideSelectionControl extends BlockControl
{
        ...
        for (int i = 0; i < selectedDirections.size(); i++)
        {
            ...
            switch (direction)
            {
                case NORTH:
                    ...
                    break;
                case EAST:
                    renderTextOnSide(s, x, y, z, scale, org.joml.Vector3f.YP.rotationDegrees(90.0f), 0xffffffff);
                    break;
                case SOUTH:
                    renderTextOnSide(s, x, y, z, scale, org.joml.Vector3f.YP.rotationDegrees(180.0f), 0xffffffff);
                    break;
                case WEST:
                    renderTextOnSide(s, x, y, z, scale, org.joml.Vector3f.YP.rotationDegrees(-90.0f), 0xffffffff);
                    break;
                case UP:
                    renderTextOnSide(s ,x, y, z, scale, org.joml.Vector3f.XP.rotationDegrees(90.0f), 0xffffffff);
                    break;
                case DOWN:
                    renderTextOnSide(s, x, y, z, scale, org.joml.Vector3f.XP.rotationDegrees(-90.0f), 0xffffffff);
                    break;
        ...
        if (mouseOverDirection != null)
        {
            switch (mouseOverDirection)
            {
                case NORTH:
                    ...
                    break;
                case SOUTH:
                    renderRectangleOnSide(x, y, z, scale, org.joml.Vector3f.YP.rotationDegrees(180.0f), 0x55ffffff);
                    break;
                case WEST:
                    renderRectangleOnSide(x, y, z, scale, org.joml.Vector3f.YP.rotationDegrees(-90.0f), 0x55ffffff);
                    break;
                case EAST:
                    renderRectangleOnSide(x, y, z, scale, org.joml.Vector3f.YP.rotationDegrees(90.0f), 0x55ffffff);
                    break;
                case UP:
                    renderRectangleOnSide(x, y, z, scale, org.joml.Vector3f.XP.rotationDegrees(90.0f), 0x55ffffff);
                    break;
                case DOWN:
                    renderRectangleOnSide(x, y, z, scale, org.joml.Vector3f.XP.rotationDegrees(-90.0f), 0x55ffffff);
                    break;
    ...
    private void renderTextOnSide(String text, float x, float y, float z, float scale, @Nonnull Quaternion rotation, int colour)
    {
        ...
        GuiGraphics matrixStack = new GuiGraphics();
    ...
    public Direction getDirectionMouseIsOver()
    {
        ...
        GuiGraphics pointGuiGraphics = new GuiGraphics();
        pointGuiGraphics.translate(x, y, z);
        pointGuiGraphics.mulPose(new Quaternion(rotationQuat.i(), -rotationQuat.j(), rotationQuat.k(), -rotationQuat.r()));
        GuiGraphics normalGuiGraphics = new GuiGraphics();
        normalGuiGraphics.mulPose(new Quaternion(rotationQuat.i(), -rotationQuat.j(), rotationQuat.k(), -rotationQuat.r()));
        ...
        frontPlanePoint.transform(pointGuiGraphics.last().pose());
        frontPlaneNormal.transform(normalGuiGraphics.last().pose());
        backPlanePoint.transform(pointGuiGraphics.last().pose());
        backPlaneNormal.transform(normalGuiGraphics.last().pose());
        topPlanePoint.transform(pointGuiGraphics.last().pose());
        topPlaneNormal.transform(normalGuiGraphics.last().pose());
        bottomPlanePoint.transform(pointGuiGraphics.last().pose());
        bottomPlaneNormal.transform(normalGuiGraphics.last().pose());
        leftPlanePoint.transform(pointGuiGraphics.last().pose());
        leftPlaneNormal.transform(normalGuiGraphics.last().pose());
        rightPlanePoint.transform(pointGuiGraphics.last().pose());
        rightPlaneNormal.transform(normalGuiGraphics.last().pose());
