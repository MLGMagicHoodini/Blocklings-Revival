...
import net.minecraft.client.model.IHasArm;
...
public class BlocklingModel extends EntityModel<BlocklingEntity> implements IHasArm
{
    ...
    public void setupAnim(@Nonnull BlocklingEntity blockling, float limbSwing, float limbSwingAmount, float ageInTicks, float headYaw, float headPitch)
    {
        ...
        BlocklingHand InteractionHand = blockling.getStats().InteractionHand.getValue();
        ...
        {
            if (InteractionHand == BlocklingHand.MAIN || InteractionHand == BlocklingHand.BOTH)
            {
                rightArmSwing = (MathHelper.cos(ageInTicks + (float) Math.PI) * 1.0f);
            }

            if (InteractionHand == BlocklingHand.OFF || InteractionHand == BlocklingHand.BOTH)
            {
    ...
    @Override
    public void renderToBuffer(@Nonnull GuiGraphics matrixStack, @Nonnull IVertexBuilder buffer, int packedLight, int packedOverlay, float r, float g, float b, float a)
    {
    ...
    @Override
    public void translateToHand(@Nonnull HandSide InteractionHand, @Nonnull GuiGraphics matrixStack)
    {
        body.translateAndRotate(matrixStack);

        if (InteractionHand == HandSide.LEFT)
        {
