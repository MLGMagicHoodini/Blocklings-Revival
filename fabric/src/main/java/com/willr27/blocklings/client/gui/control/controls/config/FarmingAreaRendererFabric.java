...
/**
 * Fabric equivalent of the NeoForge {@code FarmingAreaRenderer}: draws a translucent blue "ghost"
 * preview of a blockling's farming area in the world (a filled block on each selected corner plus
 * the full X/Z rectangle between them) so the player can read the length and width at a glance.
 * Rendered for the local player's own blocklings, updating live while corners are selected.
 */
public final class FarmingAreaRendererFabric
{
    ...
    private static void onRenderLevel(@Nonnull WorldRenderContext context)
    {
        ...
        // filled-box buffer with the lines buffer throws "Not building!". Two clean passes avoid that.
